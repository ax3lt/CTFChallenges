...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            # Query parametrizzata: qui il login non è vulnerabile,
            # ma la vera vulnerabilità è nel cookie.
            ...
            if user:
                ...
                flash(f"Credenziali errate (qua injection non funziona...)", "danger")
...
def register():
    if request.method == 'POST':
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            # Query volutamente vulnerabile (anche se il focus è il cookie)
...
def profile():
    ...
    if not uid:
        flash("Cookie uid non trovato. Effettua il login.", "danger")
        return redirect('/login')
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True, buffered=True)
        # Vulnerabilità: l'ID preso dal cookie viene usato direttamente nella query!
        query = "SELECT username FROM users WHERE id = " + filter_query(uid)
        cursor.execute(query)
        user = cursor.fetchone()
        cursor.close()
        conn.close()
